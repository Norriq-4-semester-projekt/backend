...
using WorkerService.Entities.Memory;
...
namespace WorkerService.Services
{
    internal class MemorySpikeDetection : IHostedService, IDisposable
    {
        ...
        public Task StartAsync(CancellationToken stoppingToken)
        {
            ...
            if (data != null)
                foreach (var item in data)
                {
                    Data memoryData = new()
                    {
                        Value = item.System.Memory.Actual.Used.Bytes,
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync("https://localhost:5000/v1/SpikeDetection/GetLatestMemoryData");
